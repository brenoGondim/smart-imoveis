...
export class CadastroImovelComponent implements OnInit {
  ...
  buscarFotos(){
    ...
    this.commomService.getFotosImovel(`${environment.urlFotosImovel}`, this.idImovelEdited).subscribe(async response =>{
      ...
      if(this.montarFotos(this.listFotosImovel)){
        this.preencheFotosImovel = (await this.montarFotos(this.listFotosImovel)).sort((a,b) => {
          if(a.ordem  > b.ordem){
            return 1;
          }
          if(a.ordem  < b.ordem){
         ...
         this.preencheFotosImovel.forEach((foto, i) => {
          if(foto.ordem == 0){
            this.fotoPrincipal = foto.endereco;
            this.preencheFotosImovel.splice(i, 1);
          }
        });
      ...
      if(this.montarFotos(this.listFotosAreaImovel)){
        this.preencheAreasImovel = (await this.montarFotos(this.listFotosAreaImovel)).sort((a,b) => {
          if(a.ordem  > b.ordem){
            return 1;
          }
          if(a.ordem  < b.ordem){
      ...
      if(this.montarFotos(this.listFotosPlantaImovel)){
        this.preenchePlantasImovel = (await this.montarFotos(this.listFotosPlantaImovel)).sort((a,b) => {
          if(a.ordem  > b.ordem){
            return 1;
          }
          if(a.ordem  < b.ordem){
  ...
  async montarFotos(list: any): Promise<OrdemFotos[]>{
    if(list.length){   
      ...
      for (let i = 0; i < list.length; i++) {
        ...
        ordem.id = list[i].id;
        ordem.endereco = await this.addWaterMark(list[i].caminho);
  ...
  async addWaterMark(image): Promise<any>{    
    var data;    
    await watermark([image, 'assets/img/demo/marca-dagua.png'])
