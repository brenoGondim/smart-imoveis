...
export class CadastroEdificioCondominioComponent implements OnInit {
  ...
  async montarFotosImovel(list: any){
    if(list.length){      
      for (let i = 0; i < list.length; i++) {
        //if(i==9)break;
        let edificio = new OrdemFotos();
        edificio.id = list[i].id;
        edificio.endereco = await this.addWaterMark(list[i].caminho);
  ...
  async montarPlantas(list: any){
    if(list.length){
      for (let i = 0; i < list.length; i++) {
        //if(i==9)break;
        let planta = new OrdemFotos();
        planta.id = list[i].id;
        planta.endereco = await this.addWaterMark(list[i].caminho);
